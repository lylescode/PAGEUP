...
- (CGFloat)columnWidth
{
    float retVal = ((self.collectionView.bounds.size.width - 20) - (kItemLineSpacing * (self.columnsQuantity - 1))) / self.columnsQuantity;
...
-(void)prepareLayout{
    ...
    for (NSUInteger i = 0; i < itemsCount; i++){
        ...
        NSUInteger xSpacing = (kItemSpacing * columnIndex) + kLeftMargin;
        NSUInteger xOffset = (columnIndex * [self columnWidth]) + xSpacing;
        NSUInteger yOffset = [[_columns objectAtIndex:columnIndex] integerValue] + kItemLineSpacing;
        
        NSUInteger itemWidth = 0;
        NSUInteger itemHeight = 0;
        CGFloat itemRelativeHeight = [self.delegate collectionView:self.collectionView relativeHeightForItemAtIndexPath:indexPath];
        ...
        if ([self canUseDoubleColumnOnIndex:columnIndex] &&
            ...
            itemHeight = ceil(itemRelativeHeight * itemWidth);
